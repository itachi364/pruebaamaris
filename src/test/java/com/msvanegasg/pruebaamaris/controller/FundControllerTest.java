...
public class FundControllerTest {
	...
	@BeforeEach
	void setUp() {
		mockResponse = new FundResponseDTO("F001", "Fondo Ejemplo", 1500);
	}

	@Test
	void testCreateFund() throws Exception {
		FundRequestDTO request = new FundRequestDTO("Fondo Ejemplo", 1500);

		when(fundService.create(any(FundRequestDTO.class))).thenReturn(mockResponse);

		mockMvc.perform(post("/api/funds").contentType(MediaType.APPLICATION_JSON)
				.content(objectMapper.writeValueAsString(request))).andExpect(status().isOk())
				.andExpect(jsonPath("$.id").value("F001")).andExpect(jsonPath("$.name").value("Fondo Ejemplo"))
	...
	void testGetAllFunds() throws Exception {
		...
		mockMvc.perform(get("/api/funds")).andExpect(status().isOk()).andExpect(jsonPath("$[0].id").value("F001"))
	...
	void testGetById() throws Exception {
		...
		mockMvc.perform(get("/api/funds/F001")).andExpect(status().isOk()).andExpect(jsonPath("$.id").value("F001"));
	}
	...
	void testDeleteFund() throws Exception {
		...
		mockMvc.perform(delete("/api/funds/F001")).andExpect(status().isNoContent());
	}

	@Test
	void testCreateFundWithBlankName() throws Exception {
		FundRequestDTO invalidRequest = new FundRequestDTO("   ", 1500); // Nombre en blanco
	...
	void testCreateFundWithInvalidMinimumAmount() throws Exception {
		FundRequestDTO invalidRequest = new FundRequestDTO("Fondo Test", 0); // Monto inválido
	...
	void testUpdateFund() throws Exception {
		FundRequestDTO updateDto = new FundRequestDTO("Updated Name", 2500);
		FundResponseDTO updatedResponse = new FundResponseDTO("F001", "Updated Name", 2500);

		when(fundService.update(Mockito.eq("F001"), any(FundRequestDTO.class))).thenReturn(updatedResponse);

		mockMvc.perform(put("/api/funds/F001").contentType(MediaType.APPLICATION_JSON)
				.content(objectMapper.writeValueAsString(updateDto))).andExpect(status().isOk())
				.andExpect(jsonPath("$.id").value("F001")).andExpect(jsonPath("$.name").value("Updated Name"))
				.andExpect(jsonPath("$.minimumAmount").value(2500));
	}

	@Test
	void testUpdateFund_NotFound() throws Exception {
		FundRequestDTO request = new FundRequestDTO("Name", 1000);

		when(fundService.update(Mockito.eq("F999"), any(FundRequestDTO.class)))
				.thenThrow(new RuntimeException("Fund not found"));

		mockMvc.perform(put("/api/funds/F999").contentType(MediaType.APPLICATION_JSON)
